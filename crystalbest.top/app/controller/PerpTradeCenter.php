...
use app\service\Perp\PerpTradingService;
use think\facade\Log;

final class PerpTradeCenter extends BaseController
{
    public function account()
    {
        return $this->handle(function (PerpTradingService $service) {
            return json([
                'code' => 0,
                'message' => 'success',
    ...
    public function closePosition(string $position)
    {
        return $this->handle(function (PerpTradingService $service) use ($position) {
            $result = $service->closePosition((int) $position);
            return json([
                'code' => 0,
    ...
    {
        return $this->handle(function (PerpTradingService $service) {
            $result = $service->setLeverage($this->payload());
            return json([
                'code' => 0,
                'message' => (string) $result['message'],
                'data' => $result,
    ...
    {
        return $this->handle(function (PerpTradingService $service) use ($order) {
    ...
    private function handle(callable $callback)
    {
        try {
            ...
            return $callback(new PerpTradingService($this->request));
            ...
            Log::error(
                'Perp trade center error exception=' . get_class($exception)
                . ' message=' . $exception->getMessage()
                . ' file=' . $exception->getFile()
                . ' line=' . $exception->getLine()
            );
            return json([
                'code' => 'PERP_INTERNAL_ERROR',
                'message' => '合约交易服务暂时不可用，请稍后重试',
                'data' => null,
            ], 500);
        }
    }
